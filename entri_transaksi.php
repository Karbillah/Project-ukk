        ...
        <div class="widget-box">
          ...
          <div class="widget-content nopadding" >
            <table class="table table-bordered table-invoice-full">
              ...
              <tbody>
                <?php
                  // Contoh query pada bagian yang menampilkan data "Belum Bayar"
                  $query_order = "SELECT * FROM `order` LEFT JOIN user ON `order`.id_user = user.id_user WHERE status_order = 'belum bayar'";
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <?php
                      if ($r['id_level'] == 3) {
                        // Hanya render tombol "Transaksi" untuk level 3
                        echo '<button type="submit" value="' . $r_order['id_order'] . '" name="edit_order" class="btn btn-success btn-mini"><i class="icon-pencil"></i>&nbsp;&nbsp;Transaksi&nbsp;&nbsp;</button>';
                      }
                
                      // Tombol "Hapus" selalu ditampilkan
                      echo '<button type="submit" value="' . $r_order['id_order'] . '" name="hapus_order" class="btn btn-mini btn-danger"><i class="icon icon-trash"></i>&nbsp; Hapus</button>';
                    ?>
                    </form>
                  </td>
                </tr>
                <?php
                  ...
                  if(isset($_REQUEST['hapus_order'])){
                    ...
                    $query_hapus_order = "delete from `order` where id_order = $id_order";
                    $query_hapus_pesan_order = "delete from pesan where id_order = $id_order";
          ...
          <div class="widget-content nopadding" >
            <table class="table table-bordered table-invoice-full">
              ...
              <tbody>
                <?php
                  $nomor = 1;
                  $query_sudah_order = "select * from `order` left join user on `order`.id_user = user.id_user where status_order = 'sudah bayar' order by id_order desc";
