...
<div id="user-nav" class="navbar navbar-inverse">
  <ul class="nav">
    <li class="dropdown" id="profile-messages"><a title="" href="#" data-toggle="dropdown" data-target="#profile-messages" class="dropdown-toggle"><i class="icon icon-user"></i> <span class="text">Selamat Datang <?php echo $r['nama_user']; ?></span><b class="caret"></b></a>
...
<!--breadcrumbs-->
  <div id="content-header">
    <div id="breadcrumb">
      <a href="entri_transaksi.php" title="Entri Transaksi" class="tip-bottom"><i class="icon icon-inbox"></i>
        Entri Transaksi
      </a>
  ...
  <div class="container-fluid">
    ...
    <?php
      if ($r['id_level'] == 3) {
        ...
        $query_pemesan = "SELECT * FROM `order` LEFT JOIN user ON `order`.id_user = user.id_user WHERE id_order = $id_order";
        $sql_pemesan = mysqli_query($conn, $query_pemesan);
        $result_pemesan = mysqli_fetch_array($sql_pemesan);
        $id_pemesan = $result_pemesan['id_user'];
    ?>
      <div class="span7">
          ...
          <div class="widget-content nopadding">
            <table class="table table-bordered table-invoice-full">
              ...
              <tbody>
                <?php
                  $no_order_fiks = 1;
                  $query_order_fiks = "SELECT * FROM pesan LEFT JOIN masakan ON pesan.id_masakan = masakan.id_masakan WHERE id_user = $id_pemesan AND status_pesan != 'sudah'";
                ...
                <?php
                  ...
                  $query_harga = "SELECT * FROM `order` WHERE id_user = $id_pemesan AND status_order = 'belum bayar'";
      ...
      <?php
          ...
          if (isset($_REQUEST['save_order'])) {
            ...
            $query_save_transaksi = "UPDATE `order` SET id_admin = $id, uang_bayar = $uang_bayar, uang_kembali = $uang_kembali, status_order = 'sudah bayar' WHERE id_order = $id_order";
            $sql_save_transaksi = mysqli_query($conn, $query_save_transaksi);

            $query_selesai_pesan = "UPDATE pesan SET status_pesan = 'sudah' WHERE id_user = $id_pemesan AND status_pesan != 'sudah'";
